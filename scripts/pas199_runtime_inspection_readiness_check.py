"""
PAS199 — Operational runtime inspection readiness gate.

Deterministic, read-only evaluator for "is PAS199 wired correctly,
additive-only over PAS198, deterministic, free of
...
Walks the repo and verifies:
  ...
  * PAS199 surfaces exist (doc / readiness gate / CLI / service /
    test).
  * Inspection service exposes the lineage reconstruction,
    transcript replay, and runtime comparison entry points.
  ...
  * Runtime smoke: build_inspection on a synthetic clean lineage
    returns the required keys, allowed_environment=SIMULATION_ONLY,
    live_behavior_changed=False, lineage_intact=True.
  * No new SQL migration introduced under scripts/ whose filename
    carries "pas199".
  * combined_supabase_migration.sql is not committed on this
    branch.
  * No secret-shaped tokens in any PAS199 file.
  * CLI refuses to write outside reports/simulations/.
  * The readiness gate itself never reads .env / hits the network.
  * PAS193 / PAS194 / PAS195 / PAS196 / PAS197 / PAS198 carry-
    forward intact.
...
DOC_FILE = "docs/pas199_runtime_inspection.md"
...
def check_pas199_files_present(repo_root: str) -> List[dict]:
    out: List[dict] = []
    for rel in PAS199_FILES:
        ok = (Path(repo_root) / rel).is_file()
        out.append(_check(
            f"files:exists:{rel}",
            "PASS" if ok else "FAIL",
            f"PAS199 artefact present: {rel}",
...
def check_required_entry_points_present(repo_root: str) -> List[dict]:
    src = _read_text(
        Path(repo_root) / "app/services/simulation/runtime_inspection.py",
    ) or ""
    names = _collect_identifier_names(src)
    required = (
        ("entry_point:build_inspection",
         "build_inspection", "lineage reconstruction entry point"),
        ("entry_point:replay_transcript",
         "replay_transcript", "transcript replay helper"),
        ("entry_point:compare_runtimes",
         "compare_runtimes", "runtime comparison helper"),
    )
    return [
        _check(
            cid,
            "PASS" if name in names else "FAIL",
            f"runtime_inspection exposes {desc}",
...
def check_required_status_constants_present(repo_root: str) -> List[dict]:
    ...
    required = (
        ("status:phase_pas199",
         "PAS199", "PAS199 phase identifier"),
    ...
    return [
        _check(
            ...
            f"runtime_inspection declares {desc}",
...
def check_no_new_migration(repo_root: str) -> List[dict]:
    ...
    return [_check(
        "no_new_migration",
        "FAIL" if bad else "PASS",
        "PAS199 introduces no SQL migration",
...
def check_runner_writes_only_reports(repo_root: str) -> List[dict]:
    fp = Path(repo_root) / "scripts" / "pas199_inspect_runtime_lineage.py"
...
def check_inspection_smoke(repo_root: str) -> List[dict]:
    out: List[dict] = []
    ...
    try:
        ...
        insp = build_inspection(
            rec, rev, pkg, rt, generated_at="2026-05-22T00:00:00Z",
        )
        missing = [k for k in INSPECTION_REQUIRED_KEYS if k not in insp]
        out.append(_check(
            "inspection:required_keys",
            "PASS" if not missing else "FAIL",
            "inspection artefact carries every required key",
            detail=("missing: " + ", ".join(missing)) if missing else "",
        ))
        out.append(_check(
            "inspection:allowed_environment_simulation_only",
            "PASS" if insp["allowed_environment"] == INSPECTION_ENVIRONMENT_SIMULATION_ONLY else "FAIL",
            "inspection artefact allowed_environment is SIMULATION_ONLY",
            detail="" if insp["allowed_environment"] == INSPECTION_ENVIRONMENT_SIMULATION_ONLY else f"got {insp['allowed_environment']!r}",
        ))
        out.append(_check(
            "inspection:live_behavior_changed_false",
            "PASS" if insp["live_behavior_changed"] is False else "FAIL",
            "inspection artefact live_behavior_changed is False",
            detail="" if insp["live_behavior_changed"] is False else "value not False",
        ))
        out.append(_check(
            "inspection:inspection_id_shape",
            "PASS" if str(insp["inspection_id"]).startswith("pas199-insp-") else "FAIL",
            "inspection_id carries pas199 prefix",
            detail=str(insp["inspection_id"]),
        ))
        out.append(_check(
            "inspection:lineage_intact",
            "PASS" if insp["lineage_summary"]["lineage_intact"] is True else "FAIL",
            "lineage marked intact on clean four-artefact chain",
            detail="" if insp["lineage_summary"]["lineage_intact"] is True
                       else "lineage_intact not True",
        ))
        # Replay smoke
        first_sid = rt["transcript_bundle"][0]["scenario_id"]
        replay = replay_transcript(rt, first_sid)
        out.append(_check(
            "inspection:replay_returns_sequence_ids",
            "PASS" if replay["sequence_ids"] else "FAIL",
            "replay_transcript returns a non-empty sequence",
            detail="" if replay["sequence_ids"] else "sequence empty",
        ))
        # Comparison smoke (compare against self -> zero deltas)
        cmp = compare_runtimes(rt, rt)
        zero_ok = (
            cmp["pass_rate_delta"] == 0.0
            and cmp["average_score_delta"] == 0.0
            and cmp["safety_delta"]["auto_fail_count_delta"] == 0
            and cmp["transcript_size_delta"]["total_turns_delta"] == 0
            and cmp["flipped_scenarios"] == []
        )
        out.append(_check(
            "inspection:compare_self_zero_deltas",
            "PASS" if zero_ok else "FAIL",
            "compare_runtimes(rt, rt) returns zero deltas",
            detail="" if zero_ok else json.dumps(cmp, sort_keys=True)[:200],
        ))
    except Exception as e:
        out.append(_check(
            "inspection:smoke",
            "FAIL",
            "runtime_inspection module importable and runnable",
...
def check_self_no_env_or_network(repo_root: str) -> List[dict]:
    ...
    return [_check(
        ...
        "FAIL" if bad else "PASS",
        "PAS199 readiness checker never reads .env / touches DB / hits network",
        detail=("disqualifying: " + ", ".join(sorted(set(bad)))) if bad else "",
    )]
...
def evaluate(repo_root: str) -> dict:
    checks: List[dict] = []
    checks.extend(check_pas199_files_present(repo_root))
    ...
    checks.extend(check_inspection_smoke(repo_root))
...
REPORT_FILENAME = "pas199_runtime_inspection_readiness_report.json"


def _build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        prog="pas199_runtime_inspection_readiness_check",
        description=(
            "PAS199 — Operational Runtime Inspection readiness gate. "
...
def _print_summary(report: dict) -> None:
    print(
        f"[PAS199] verdict={report['verdict']} "
