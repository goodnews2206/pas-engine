"""
PAS197 — Manual-test package readiness gate.

Deterministic, read-only evaluator for "is PAS197 wired correctly,
additive-only over PAS196, deterministic, free of Twilio/Slack/LLM
dependencies, free of forbidden status literals or live-mutation
identifiers, structurally bounded to SIMULATION_ONLY, and not
introducing any new SQL migration?"

Walks the repo and verifies:

  * PAS197 surfaces exist (doc / readiness gate / CLI / service /
    test).
  * Package service exposes STATUS_READY_FOR_MANUAL_TEST =
    "READY_FOR_MANUAL_TEST" and
    ALLOWED_ENVIRONMENT_SIMULATION_ONLY = "SIMULATION_ONLY".
  * Service + CLI source string-constants do not contain the
    ...
    APPROVED_FOR_MANUAL_TEST and READY_FOR_MANUAL_TEST are
    unaffected).
  ...
  * Runtime smoke: build_manual_test_package on a synthetic
    approved pair returns status=READY_FOR_MANUAL_TEST,
...
DOC_FILE = "docs/pas197_manual_test_package.md"
...
def check_pas197_files_present(repo_root: str) -> List[dict]:
    out: List[dict] = []
    for rel in PAS197_FILES:
        ok = (Path(repo_root) / rel).is_file()
        out.append(_check(
            f"files:exists:{rel}",
            "PASS" if ok else "FAIL",
            f"PAS197 artefact present: {rel}",
...
def check_required_status_constants_present(repo_root: str) -> List[dict]:
    src = _read_text(
        Path(repo_root) / "app/services/simulation/manual_test_package.py",
    ) or ""
    consts = _collect_string_constants(src)
    return [
        _check(
            "status:ready_for_manual_test_present",
            "PASS" if "READY_FOR_MANUAL_TEST" in consts else "FAIL",
            "package service declares READY_FOR_MANUAL_TEST",
        ...
        _check(
            ...
            "package service declares SIMULATION_ONLY",
...
def check_no_new_migration(repo_root: str) -> List[dict]:
    ...
    return [_check(
        "no_new_migration",
        "FAIL" if bad else "PASS",
        "PAS197 introduces no SQL migration",
...
def check_runner_writes_only_reports(repo_root: str) -> List[dict]:
    fp = Path(repo_root) / "scripts" / "pas197_create_manual_test_package.py"
...
def check_runtime_package_smoke(repo_root: str) -> List[dict]:
    out: List[dict] = []
    sys.path.insert(0, repo_root)
    try:
        from app.services.simulation.manual_test_package import (  # type: ignore
        ...
        missing = [k for k in PACKAGE_REQUIRED_KEYS if k not in pkg]
        out.append(_check(
            "runtime:package_required_keys",
            "PASS" if not missing else "FAIL",
            "package carries every required key",
            detail=("missing: " + ", ".join(missing)) if missing else "",
        ))
        out.append(_check(
            "runtime:status_ready_for_manual_test",
            "PASS" if pkg["status"] == STATUS_READY_FOR_MANUAL_TEST else "FAIL",
            "runtime package status is READY_FOR_MANUAL_TEST",
            detail="" if pkg["status"] == STATUS_READY_FOR_MANUAL_TEST else f"got {pkg['status']!r}",
        ))
        out.append(_check(
            "runtime:allowed_environment_simulation_only",
            "PASS" if pkg["allowed_environment"] == ALLOWED_ENVIRONMENT_SIMULATION_ONLY else "FAIL",
            "runtime package allowed_environment is SIMULATION_ONLY",
            detail="" if pkg["allowed_environment"] == ALLOWED_ENVIRONMENT_SIMULATION_ONLY else f"got {pkg['allowed_environment']!r}",
        ))
        out.append(_check(
            "runtime:live_behavior_changed_false",
            "PASS" if pkg["live_behavior_changed"] is False else "FAIL",
            "runtime package live_behavior_changed is False",
            detail="" if pkg["live_behavior_changed"] is False else "value not False",
        ))
        out.append(_check(
            "runtime:package_id_shape",
            "PASS" if str(pkg["package_id"]).startswith("pas197-pkg-") else "FAIL",
            "package_id carries pas197 prefix",
            detail=str(pkg["package_id"]),
        ))
    except Exception as e:
        out.append(_check(
            "runtime:package_smoke",
            "FAIL",
            "manual_test_package module importable and runnable",
...
def check_self_no_env_or_network(repo_root: str) -> List[dict]:
    ...
    return [_check(
        ...
        "FAIL" if bad else "PASS",
        "PAS197 readiness checker never reads .env / touches DB / hits network",
        detail=("disqualifying: " + ", ".join(sorted(set(bad)))) if bad else "",
    )]
...
def evaluate(repo_root: str) -> dict:
    checks: List[dict] = []
    checks.extend(check_pas197_files_present(repo_root))
    ...
    checks.extend(check_runtime_package_smoke(repo_root))
...
REPORT_FILENAME = "pas197_manual_test_package_readiness_report.json"


def _build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        prog="pas197_manual_test_package_readiness_check",
        description=(
            "PAS197 — Manual-test Package readiness gate. "
...
def _print_summary(report: dict) -> None:
    print(
        f"[PAS197] verdict={report['verdict']} "
